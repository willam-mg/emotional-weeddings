...
//phpcs:disable
/**
 * This file is a copy of WordPress' wp-admin/includes/export.php with an extension to allow 
 * filtering for post ids via the $args['post_ids']. If at some point WordPress updates their 
 * exporter, we need to update this file as well.
 */
 ...
 * }
 */
if (!function_exists('dipi_popup_maker_export_wp')) {
	function dipi_popup_maker_export_wp($args = array())
	{
		global $wpdb, $post;
		...
		if (isset($args['post_ids']) && is_array($args['post_ids'])) {
			$esses = array_fill(0, count($args['post_ids']), '%s');
			$where .= $wpdb->prepare(" AND {$wpdb->posts}.ID IN (" . implode(',', $esses) . ')', $args['post_ids']);
		}
		 ...
		 * @param WP_Term $term Term object.
		 */
		if (!function_exists('wxr_term_meta')) {
		 ...
		 * @since 3.1.0
		 */
		if (!function_exists('wxr_nav_menu_terms')) {
		...
		<rss version="2.0" xmlns:excerpt="http://wordpress.org/export/<?php echo WXR_VERSION; ?>/excerpt/"
			xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:wfw="http://wellformedweb.org/CommentAPI/"
			xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:wp="http://wordpress.org/export/<?php echo WXR_VERSION; ?>/">

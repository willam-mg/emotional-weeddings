...
function socialfeeds_pro_updater_filter_args($queryArgs){
	
	global $socialfeeds;
	
	if (!empty($socialfeeds->license['license'])){
		$queryArgs['license'] = $socialfeeds->license['license'];
	}
...
function socialfeeds_pro_updater_check_link($final_link){
	
	global $socialfeeds;
	
	if(empty($socialfeeds->license['license'])){
		return '<a href="'.admin_url('admin.php?page=socialfeeds-license').'">Install SocialFeeds Pro License Key</a>';
	}
...
function socialfeeds_pro_file_get_free_version_num(){
		
	if(defined('SOCIALFEEDS_VERSION')){
		return SOCIALFEEDS_VERSION;
	}
	
	// In case of Socialfeeds deactive
	return socialfeeds_pro_file_get_version_num('socialfeeds/socialfeeds.php');
}

// Prevent update of Socialfeeds free
function socialfeeds_pro_file_get_version_num($plugin){
...
// Prevent update of Socialfeeds free
function socialfeeds_pro_disable_manual_update_for_plugin($transient){
	$plugin = 'socialfeeds/socialfeeds.php';
	...
	$free_version = socialfeeds_pro_file_get_free_version_num();
	$pro_version = SOCIALFEEDS_PRO_VERSION;
	
	if(!empty($GLOBALS['socialfeeds_pro_is_upgraded'])){
		$pro_version = socialfeeds_pro_file_get_version_num('socialfeeds-pro/socialfeeds-pro.php');
	}

	// Update the socialfeeds version to the equivalent of Pro version
	if(!empty($pro_version) && version_compare($free_version, $pro_version, '<')){
		$transient->response[$plugin]->new_version = $pro_version;
		$transient->response[$plugin]->package = 'https://downloads.wordpress.org/plugin/socialfeeds.'.$pro_version.'.zip';
	}else{
...
function socialfeeds_pro_update_free_after_pro($upgrader_object, $options){
	...
	$free_slug = 'socialfeeds/socialfeeds.php'; 
	$pro_slug = 'socialfeeds-pro/socialfeeds-pro.php';
	...
	if( 
		...
		$current_version = socialfeeds_pro_file_get_free_version_num();
		
		if(empty($current_version)){
			return;
		}
		
		$GLOBALS['socialfeeds_pro_is_upgraded'] = true;
...
// Load license data
function socialfeeds_pro_load_license($parent = 0){
	
	global $socialfeeds, $lic_resp, $sitepad;
	
	$license_field = 'socialfeeds_license';
	$license_api_url = SOCIALFEEDS_API;
	...
	if(empty($lic) || empty($lic['active'])){
		...
		if(function_exists('softaculous_pro_load_license')){
			...
			if(!empty($lic)){
				return socialfeeds_pro_load_license(1);
			}
		}
	}
	
	if(!empty($lic['license'])){
		$socialfeeds->license = $lic;
	}

	if(defined('SITEPAD') && empty($socialfeeds->license)){
		...
		$socialfeeds->license = $license;
	}

}

add_filter('softaculous_pro_products', 'socialfeeds_softaculous_pro_products', 10, 1);
function socialfeeds_softaculous_pro_products($r = []){
	$r['socialfeeds'] = 'socialfeeds';
	return $r;
}

function socialfeeds_pro_api_url($main_server = 0, $suffix = 'socialfeeds'){
	global $socialfeeds;
	...
	if(!empty($main_server) || empty($socialfeeds->license['last_edit']) || 
		(!empty($socialfeeds->license['last_edit']) && (time() - 3600) < $socialfeeds->license['last_edit'])
	){
		$mirror = SOCIALFEEDS_API;
	}
...
function socialfeeds_pro_plugin_update_notice_filter($plugins = []){
	$plugins['socialfeeds-pro/socialfeeds-pro.php'] = 'Socialfeeds Pro';
	return $plugins;
}

function socialfeeds_pro_is_network_active($pluign){

...
class SiteSEOUpdateChecker_3_2 {
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, __('Check for updates', 'siteseo-pro'));
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = __('This plugin is up to date.', 'siteseo-pro');
			} else if ( $status == 'update_available' ) {
				$message = __('A new version of this plugin is available.', 'siteseo-pro');
			} else {
				// Translators: %s is the status of the update checker.
				$message = sprintf(__('Unknown update checker status "%s"', 'siteseo-pro'), htmlentities($status));
			}
			printf(

...
trait StyleDeclarationTrait {
	...
	public static function icon_font_declaration( $params ) {
		$icon_attr_raw = $params['attrValue'] ?? [];
		
		$icon_attr = is_array($icon_attr_raw) && isset($icon_attr_raw['desktop']['value']) 
			? $icon_attr_raw['desktop']['value'] 
			: $icon_attr_raw;
		...
		if ( ! empty( $icon_attr ) && is_array( $icon_attr ) ) {
			...
			$font_weight = $icon_attr['weight'] ?? '400';
